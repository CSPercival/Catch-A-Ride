...
class PTClient:
    ...
    def _get_stop_struct(coordinate: Coordinate, name: str, arrival_time):
        stop_struct = {
            'id': 0,
            'lat': coordinate[0],
            'lng': coordinate[1],
    ...
    def get_full_route(self, start: Coordinate, end: Coordinate, start_time: int, start_name="NN", end_name="NN") -> dict:
        ...
        raw_route['stops'].insert(0, self._get_stop_struct(start, start_name, 0))
        raw_route['trips'].insert(0, self._get_trip_struct())

        raw_route['stops'].append(self._get_stop_struct(end, end_name, route_duration))
    ...
    def get_reach_time(self, start: Coordinate, end: Coordinate, start_time: int) -> List[int]:
        ...
        end_stop_walk_times = matrix_durations(self.ors_walk_client.matrix_s2d([end], get_stops_coords(end_stop_ids, self.stop_data), ['duration']))
        ...
        for end_id, end_walk, end_reach in zip(end_stop_ids, end_stop_walk_times, end_stop_reach_times):
            if ans[0] > end_walk + end_reach[0]:
                ans = [end_walk + end_reach[0], end_reach[1], end_id]
        return ans
    ...
    def get_reach_times(self, start: Coordinate, end_ids: List[int], start_time: int) -> List[List[int]]:
        ...
        start_stop_times = matrix_durations(self.ors_walk_client.matrix_s2d([start], get_stops_coords(start_stop_ids, self.stop_data), ['duration']))
    ...
    def get_reach_times_stop(self, start_ids: List[int], end_ids: List[int], start_times: List[int]) -> List[List[int]]:
        ...
        for id, start_time in zip(start_ids, start_times):
            ...
            for final_time, end_id in zip(final_reach_times, end_ids):
                if final_time[0] > single_reach_times[end_id]:
                    final_time = [single_reach_times[end_id], id]
